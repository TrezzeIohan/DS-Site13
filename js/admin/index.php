<?php
	...
	if(!isset($REQUIRED)){
		...
		$js = explode(".", $file);
		$js = end($js) === "js";
		if(preg_match("/js\/admin\/plugins.js/", $file)){
			$output = "";
			foreach(glob((new __paths)->get()->www . "/js/admin/plugins/*.js") as $file){
				$output .= file_get_contents($file);
				$output .= "\n";
			}
			exit(($output));
		}  elseif(preg_match("/js\/admin\/i18n.js/", $file)){
			$output = "";
			foreach(glob((new __paths)->get()->www . "/i18n/*/*.js") as $file){
				$output .= file_get_contents($file);
				$output .= "\n";
			}
			exit((JSMin::minify($output, array('flaggedComments' => false))));
		} elseif(file_exists($file) && $js && preg_match($exclude, $file)){
			exit(file_get_contents($file));
		} elseif(file_exists($file) && $js){
			$code = (JSMin::minify(file_get_contents($file), array('flaggedComments' => false)));
			$code = preg_replace("/    /","\\t",$code);
			$code = preg_replace("/\t/","\\t",$code);
			exit($code);
		} else {
			?>LWDK.debug.error("Arquivo não localizado: <?=$file;?>");<?php
		}
	}
?>
